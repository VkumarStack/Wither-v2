...
class HeaderBar extends React.Component {
    constructor(props) {
        super(props);
        this.state = { matches: window.matchMedia("(min-width: 800px)").matches, searchShowing: true }
    }
        ...
        if (sessionStorage.getItem("token") !== null) {
            return(
                <div className="user-buttons">
                    { (this.props.profile !== sessionStorage.getItem("user")) && <YourProfile/> }
                    <Logout/>
                </div>
            );
        } else {
        ...
        if (this.state.matches) {
            ...
            return (
                <div className="HeaderBar">
                    ...
                    { this.state.searchShowing ? <SearchBar/> : this.renderButtons()}
                    <img src={this.state.searchShowing ? RightIcon : LeftIcon} alt={this.state.searchShowing ? "Right" : "Left"}width={40} height={40} onClick={(e) => this.setState({searchShowing: !this.state.searchShowing})}/>
                </div>
            );
        }
    }
